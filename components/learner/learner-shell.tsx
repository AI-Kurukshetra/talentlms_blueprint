...
const navigation: LearnerNavItem[] = [
  { href: "/learner", label: "Dashboard", icon: LayoutDashboard },
  { href: "/learner/browse", label: "Browse Courses", icon: Compass },
  { href: "/learner/my-courses", label: "My Courses", icon: BookOpen },
  { href: "/learner/learning-paths", label: "Learning Paths", icon: RouteIcon },
    ...
    <div className="flex h-full flex-col rounded-[28px] border border-white/10 bg-white/[0.04] p-4 backdrop-blur-xl">
      <div className="flex items-center gap-3 border-b border-white/10 px-2 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Learner workspace</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-black/10 p-4">
        <p className="text-sm font-medium">Learning Hub</p>
        <p className="mt-2 text-sm leading-7 text-muted-foreground">
          Pick up where you left off, track your momentum, and turn completed work into earned certificates.
        </p>
      </div>
    </div>
  )
...
export function LearnerShell({ user, children }: LearnerShellProps) {
        ...
        <header className="glass-panel sticky top-4 z-30 mb-4 rounded-[28px] px-4 py-4 sm:px-5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm uppercase tracking-[0.22em] text-primary">Learner</p>
                <h1 className="text-lg font-semibold tracking-tight">Progress and learning hub</h1>
              </div>
            </div>
